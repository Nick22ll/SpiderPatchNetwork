...
from SpiderPatch.ReadoutMeshGraph import ReadoutMeshGraph


class ReadoutMeshGraphDataset(DGLDataset):
    def __init__(self, dataset_name="", graphs=None, labels=None):
        self.graphs = graphs
        self.labels = labels
        super().__init__(name=dataset_name)

    def fromRawPatches(self, load_path, resolution_level="all", save_dir=None):
        self.graphs = []
        self.labels = []
        if resolution_level != "all":
            for mesh in subdivide_for_mesh():
                mesh_id, class_label = mesh[resolution_level]
                with open(f"{load_path}/{class_label}/patches{mesh_id}.pkl", "rb") as pkl_file:
                    patches = pkl.load(pkl_file)
                    self.graphs.append(ReadoutMeshGraph(patches))
            ...
            for class_label in os.listdir(load_path):
                for graph_pkl in os.listdir(f"{load_path}/{class_label}"):
                    with open(f"{load_path}/{class_label}/{graph_pkl}", "rb") as pkl_file:
                        patches = pkl.load(pkl_file)
                        prova = ReadoutMeshGraph(patches)
                        self.graphs.append(prova)
                        self.labels.append(int(class_label))
...
class ReadoutMeshGraphDatasetForNNTraining:
    ...
    def loadFromDGLDataset(self, dgl_dataset_path, dgl_dataset_name):
        dataset = ReadoutMeshGraphDataset(dataset_name=dgl_dataset_name)
    ...
    def split_graphs(self, graphs, labels):
        ...
        self.train_dataset = ReadoutMeshGraphDataset(dataset_name=self.name + "_train", graphs=data_train, labels=label_train)
        ...
        self.validation_dataset = ReadoutMeshGraphDataset(dataset_name=self.name + "_val", graphs=data_validation, labels=label_validation)
        self.test_dataset = ReadoutMeshGraphDataset(dataset_name=self.name + "_test", graphs=data_test, labels=label_test)

    def loadFromRawPatchesDataset(self, patch_radius=2, rings_number=4, point_per_ring=6, Normalized=False):
        if Normalized:
            self.name = f"SHREC17_{patch_radius}_{point_per_ring}_{rings_number}_Normalized"
        else:
            self.name = f"SHREC17_{patch_radius}_{point_per_ring}_{rings_number}"
        ...
        for class_label in os.listdir(self.base_path):
            for graph_pkl in os.listdir(f"{self.base_path}/{class_label}"):
                with open(f"{self.base_path}/{class_label}/{graph_pkl}", "rb") as pkl_file:
                    patches = pkl.load(pkl_file)
                    prova = ReadoutMeshGraph(patches)
                    graphs.append(prova)

...
class SuperPatch(dgl.DGLGraph):
    def __init__(self, sample_mesh, seed_point, radius, n_rings, n_points):
        print("Calculating resolution level_0 of SuperPatch...")
        ...
        concentricRings = CSIRSv2Arbitrary(level_0, seed_point, radius, n_points, n_rings)
        ...
        node_features["f_normal"] = level_0.faces_normals()[mesh_face_idx].dot(mat_transform.T)
        ...
        for curvature in ["gauss_curvature", "mean_curvature", "curvedness", "K2", "local_depth"]:
            node_features[curvature] = level_0.face_curvatures[curvature][mesh_face_idx].reshape((-1, 1))
        ...
        print("Finish resolution level_0 of SuperPatch and structure generation...")
        del level_0
        ...
        for resolution_level in ["level_1", "level_2", "level_3"]:
            ...
            concentricRings = CSIRSv2Arbitrary(mesh, seed_point, radius, n_points, n_rings)

            mesh_face_idx = concentricRings.getNonNaNFacesIdx()
            f_normals = mesh.faces_normals()[mesh_face_idx].dot(mat_transform.T)
            node_features["f_normal"] = np.hstack((node_features["f_normal"], f_normals))

            if not mesh.has_curvatures():
                mesh.computeCurvatures(radius)
            for curvature in ["gauss_curvature", "mean_curvature", "curvedness", "K2", "local_depth"]:
                tmp_curvature = mesh.face_curvatures[curvature][mesh_face_idx].reshape((-1, 1))
                node_features[curvature] = np.hstack((node_features[curvature], tmp_curvature))

            print(f"Finish calculating resolution {resolution_level} of SuperPatch...")

...
class MeshGraphGeo(dgl.DGLGraph):
    def __init__(self, patches, mesh, neighbours_number=0, sample_id=None, mesh_id=None, resolution_level=None, keep_feats_names=None, exact_geo=False):
        """
        ...
        if neighbours_number > 0:
            neighbour_points = neighbours_number + 1
            seed_points_indices = np.zeros((len(seed_points)), dtype=int)
            for i, seed_point in enumerate(seed_points):
                seed_points_indices[i] = np.where(np.all(mesh.vertices == seed_point, axis=1))[0]

            if exact_geo:
                matrice_distanze = np.zeros((len(seed_points), len(seed_points)))
                for i in range(len(seed_points_indices)):
                    distanza = gdist.compute_gdist(mesh.vertices, mesh.faces, np.array([seed_points_indices[i]]), seed_points_indices[i + 1:])
                    matrice_distanze[i, i + 1:] = distanza
                    matrice_distanze[i + 1:, i] = distanza
            else:
                matrice_distanze, _ = mesh.geodesicApproximation(seed_points_indices, seed_points_indices)

            points_idx = np.argsort(matrice_distanze, axis=1)[:, :neighbour_points]
            start_nodes = np.hstack((start_nodes, [val for val in range(points_idx.shape[0]) for _ in range(neighbour_points - 1)]))
            end_nodes = np.hstack((end_nodes, points_idx[:, 1:].flatten()))
        else:
            for i in range(len(patches)):
